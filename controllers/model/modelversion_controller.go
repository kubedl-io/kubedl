...
func (r *ModelVersionReconciler) Reconcile(req ctrl.Request) (ctrl.Result, error) {
	...
	if err != nil {
		...
		appendModelToOwnerReference(modelVersion, model)
	}
	...
	if modelVersion.Spec.Storage == nil {
		// modelVersion doesn't have storage defined, use the storage defined from the parent model
	...
	if err != nil {
		if errors.IsNotFound(err) {
			...
			//modelCopy.Status = *modelStatusCopy
...
// The modelVersion has its own storage defined, create its own pv and pvc.
...
func (r *ModelVersionReconciler) createPVAndPVCForModelVersion(modelVersion *modelv1alpha1.ModelVersion, model *modelv1alpha1.Model,
	...
	if modelVersion.Spec.Storage.LocalStorage != nil {
		...
		if modelVersion.Spec.Storage.LocalStorage.NodeName != "" {
			...
			// get nodeName from parent model
	...
	if modelVersion.Spec.Storage.LocalStorage != nil {
		...
		// for remote storage
		pvcName = GetModelVersionPVCName(modelVersion.Name)
	}
...
func appendModelToOwnerReference(modelVersion *modelv1alpha1.ModelVersion, model *modelv1alpha1.Model) {
	// add owner reference
